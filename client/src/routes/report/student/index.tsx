...
import { fetch_grade_records_by_student_id } from "../../../modules";
...
const StudentReport = () => {
  ...
  const { toPDF, targetRef } = usePDF({
    ...
    filename: `student_report_${id}.pdf`,
  });
  ...
  useEffect(() => {
    (async () => {
      const { data, response }: { data: Grades[]; response: Response } =
        await fetch_grade_records_by_student_id(Number(id));
  ...
  return (
    <>
      <Helmet>
        <title>Student Report | Grade Manager</title>
      </Helmet>
          ...
          <table className="w-full text-sm text-left text-gray-400">
            <caption className="p-5 text-lg font-semibold text-left text-white bg-gray-800">
              <div className="flex items-center justify-between">
                <span>
                  Student Report - [{id}] {grades[0].student.name}
            ...
            <tbody>
              {grades.length > 0 ? (
                grades.map(
                  (
                    { id, score, course: { id: course_id, code, subject } },
                    index
                  ) =>
                    id && (
                      <tr
